...
# Configure logging to print to console (which Docker logs capture)
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

app = Flask(__name__)

# Read the S3 endpoint URL from an environment variable.
# In our Docker run, we will set S3_ENDPOINT_URL to "http://host.docker.internal:4566"
s3_endpoint = os.environ.get("S3_ENDPOINT_URL", "http://host.docker.internal:4566")
logger.info("Using S3 endpoint: %s", s3_endpoint)

# Initialize the boto3 S3 client with the endpoint.
s3 = boto3.client("s3", endpoint_url=s3_endpoint)

# Load the model from the local file system.
# Ensure that food11.pth is copied into the container when building the image.
try:
...
def infer():
    ...
    # Parse the S3 URI into bucket and key. parts = s3_path[5:].split("/", 1)
    ...
    logger.info("Inference successful for %s: %s with confidence %f", s3_path, predicted_class, confidence)
    return jsonify({
        "s3_path": s3_path,
